<template>
  <node :title="config.name" :show-error="showError" :content="content" :error-info="errorInfo"
        ...
        placeholder="请设置审批人" header-bgc="#ff943e" header-icon="Avatar"/>
</template>
...
export default {
      ...
      switch (config.assignedType){
        case "ASSIGN_USER":
          if (config.assignedUser.length > 0){
            let texts = []
            config.assignedUser.forEach(org => texts.push(org.nickName))
  ...
  methods: {
    getFormItemById(id){
      return flowProcess().design.formItems.find(item => item.id === id)
    },
    //校验数据配置的合法性
    validate(err){
      try {
