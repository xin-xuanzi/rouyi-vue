  ...
  <div>
    <el-form label-position="top" label-width="90px">
      <el-form-item label="⚙ 选择审批对象" prop="text" class="user-type">
        ...
        <div v-else-if="nodeProps.assignedType === 'LEADER_TOP'">
          ...
          <el-form-item label="审批终点" prop="text" class="approve-end">
            ...
            <div class="approve-end-leave" v-if="nodeProps.leaderTop.endCondition === 'LEAVE'">
              <span>第 </span>
              <el-input-number :min="1" :max="20" :step="1" size="small" v-model="nodeProps.leaderTop.level"/>
              <span> 级主管</span>
            </div>
          </el-form-item>
        </div>
        <div v-else-if="nodeProps.assignedType === 'LEADER'">
          ...
          <el-form-item label="指定主管" prop="text">
            <span>发起人的第 </span>
            <el-input-number :min="1" :max="1" :step="1" size="small"
      ...
      <el-form-item label="👤 审批人为空时" prop="text" class="line-mode">
        <el-radio-group v-model="nodeProps.nobody.type">
        ...
        <div style="margin-top: 10px" v-if="nodeProps.nobody.type === 'TO_USER'">
      ...
      <el-form-item label="⏱ 审批期限（为 0 则不生效）" prop="timeLimit">
        <el-input style="width: 180px;padding-right: 5px" placeholder="时长" type="number"
      ...
      <el-form-item label="审批期限超时后执行" prop="level" class="line-mode" v-if="nodeProps.timeLimit.timeout.value > 0">
      ...
      <el-form-item label="🙅‍ 如果审批被驳回 👇" >
        <el-radio-group v-model="nodeProps.refuse.type">
          ...
          <el-radio label="TO_SUBMITTER">驳回提交人</el-radio>
...
export default {
  ...
  methods: {
    selectUser() {
      ...
      this.selectedProp = 'assignedUser';
      this.$nextTick(() => {
    ...
    selected(select) {
      switch (this.selectedProp) {
        case 'assignedUser':
          ...
          this.nodeProps.assignedUser = [...select];
          break;
        case "nobody":
          this.nodeProps.nobody.assignedUser = [];
          this.nodeProps.nobody.assignedUser = [...select];
          break;
      }
